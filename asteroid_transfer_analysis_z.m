...
load('./results/initial_transfer_5con_hightol.mat');
...
for stage = 1:3
    ...
    fname = sprintf('./results/5constraint_transfer/hpc_stage%d_z_hightol.mat',stage);
    load(fname)
    ...
    % Control history - combine all of them into a big array
    if stage > 1
        time = [time;min_reach.time+time(end)];
        control = [control;min_reach.control];
    elseif stage == 1
        time = [time;min_reach.time];
        control = [control;min_reach.control];
    end
...
load('./results/5constraint_transfer/final_transfer_3_hightol.mat')

% plot the final leg
set(0,'CurrentFigure',fig_handle(6));
plot3(sol_output.traj(:,1),sol_output.traj(:,2),sol_output.traj(:,3),'r','linewidth',2)

time = [time;sol_output.t+time(end)];
control = [control;sol_output.control]*1e6;

% compute the final control input
% Control
set(0,'CurrentFigure',fig_handle(7));
plot(time,control(:,1),'r')
plot(time,control(:,2),'g')
plot(time,control(:,3),'b')
