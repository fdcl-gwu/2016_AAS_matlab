function [m, dmdx] = constraints_nodenom(statef,phi_d,constants)
...
m(1) = yf;
m(2) = sin(phi1)^2*(x1^2+x2^2+x3^2)-x1^2;
m(3) = sin(phi2)^2*(x2^2+x3^2+x4^2)- x2^2;
m(4) = sin(phi3)^2*(2*x3^2+2*x3*sqrt(x4^2+x3^2)+2*x4^2)-x3-sqrt(x4^2+x3^2);
